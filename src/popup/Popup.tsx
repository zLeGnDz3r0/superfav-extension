...
export default function Popup() {
  ...
  const load = useCallback(() => {
    ...
    chrome.storage.sync.get(['superfavs'], async (result) => {
      const favs: string[] = result.superfavs ?? [];
      ...
      try {
        const res = await fetch(`${API_BASE}/api/streams?users=${favs.join(',')}`);
  ...
  useEffect(() => {
    chrome.storage.sync.get([NOTIF_SETTINGS_KEY], (res) => {
      setNotifSettings(
  ...
  return (
    <div className="w-[360px] select-none bg-sf-bg font-sans text-sf-text">
      ...
      <section className="space-y-1.5 border-b border-white/[0.06] px-3 py-2">
        <div className="flex items-center justify-between gap-2">
          <span className="text-[11px] font-medium text-sf-text">Avisar en el escritorio (directo)</span>
          <button
            id="desktop-notif-toggle"
            type="button"
            role="switch"
            aria-checked={notifSettings.desktopEnabled}
            aria-label="Avisar en el escritorio cuando hay directo"
            onClick={() =>
            ...
            className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${
              notifSettings.desktopEnabled ? 'bg-sf-accent' : 'bg-white/15'
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${
                notifSettings.desktopEnabled ? 'translate-x-4' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <span className="text-[11px] font-medium text-sf-text">Avisar si cambia el título</span>
          <button
            id="title-change-toggle"
            type="button"
            role="switch"
            aria-checked={notifSettings.titleChangeEnabled}
            aria-label="Avisar si cambia el título del canal"
            onClick={() =>
              persistNotifSettings({
                ...notifSettings,
            ...
            className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${
              notifSettings.titleChangeEnabled ? 'bg-sf-accent' : 'bg-white/15'
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${
                notifSettings.titleChangeEnabled ? 'translate-x-4' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
